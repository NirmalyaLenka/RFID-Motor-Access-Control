 ...
 *  Motor     → D5   (via Driver - Clockwise on Authorized)
 *
...
#define LED_GREEN    7
#define LED_RED      6
#define MOTOR_PIN    5   // DC Motor (via driver) - Clockwise on authorized access
...
void loop() {
  ...
  if (isAuthorized) {
    ...
    digitalWrite(MOTOR_PIN, HIGH);   // Motor ON (clockwise)
    doubleBeep();
    delay(DISPLAY_HOLD_MS);
    digitalWrite(LED_GREEN, LOW);
